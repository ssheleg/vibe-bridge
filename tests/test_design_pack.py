...
# --------------------------------------------------------------------------
# V-12 и V-13: длинное содержимое не ломает раскладку и ведёт себя одинаково.
# --------------------------------------------------------------------------


def test_a_long_line_cannot_stretch_a_flex_row():
    """Флекс-элемент по умолчанию не сжимается уже содержимого: одна длинная
    строка события распирала журнал и тянула горизонтальный скролл на всю
    панель. Соседний `.kv .v` это правило уже имел — знали и применили в
    одном месте из двух (V-12)."""
    css = _css("index.html").replace(" ", "")
    assert ".row>*{min-width:0}" in css.replace("\n", ""), (
        "дети строки журнала снова могут распереть её содержимым")
    assert "overflow-wrap:anywhere" in css, (
        "неразрывный URL всё равно шире колонки — нужен перенос внутри слова")


def test_the_reply_behaves_the_same_on_both_surfaces():
    """Одна реплика — одна картина. Виджет ограничивал длинный ответ и
    форматировал его, панель не ограничивала и показывала сырую разметку."""
    panel = _css("index.html").split(".mascot-bubble{", 1)
    assert len(panel) == 2, "пузыря панели нет"
    body = panel[1].split("}", 1)[0].replace(" ", "")
    assert declared(body, "max-height"), (
        "длинный ответ в панели ничем не ограничен, а в виджете ограничен")
    assert declared(body, "overflow-y"), "ограничение без прокрутки прячет текст"
    # Речь робота — не подпись карточки.
    assert "font-size:var(--t-cap)" not in body, (
        "речь робота идёт размером подписей карточек")


def test_the_reply_markup_has_one_implementation():
    """Две копии форматирования — это две картины одной реплики."""
    js = code_of("mascot.js")
    assert js.count("replace(/\\*\\*(") == 1, (
        "разметка ответа объявлена не один раз")
