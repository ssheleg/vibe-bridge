...
BORN_AT_RUNTIME = {
    ...
    "state.json": "состояние моста — создаётся при первом запуске в "
                  "Application Support; в репозитории его нет и быть не "
                  "должно, там ключ панели и токен робота",
}
...
# ── борд не должен худеть молча ────────────────────────────────────────────

def test_the_board_still_holds_every_finding_id():
    """2026-09-02 я срезал 127 строк борда одним скриптом: переиспользовал
    смещения `i`/`j` ПОСЛЕ того, как строка уже изменилась. Пропали девять
    находок угла архитектуры, весь угол визуала, весь угол сценариев и
    раздел «Порядок работ». Коммит прошёл зелёным — набор про борд ничего не
    знал, а глазами такое не видно: файл на 76 КБ.

    Здесь минимум, который обязан быть на месте. Не «сколько строк» — это
    ломалось бы от каждой правки, — а ЧТО именно: идентификаторы находок
    аудита и разделы, на которые ссылается всё остальное.
    """
    board = (REPO / "docs" / "evidence" / "backlog.md").read_text(
        encoding="utf-8")
    missing = []
    for prefix, upto in (("A-", 43), ("F-", 13), ("V-", 15), ("U-", 18)):
        for n in range(1, upto + 1):
            if f"| {prefix}{n} |" not in board:
                missing.append(f"{prefix}{n}")
    assert not missing, (
        "из борда пропали строки находок — почти наверняка срез скриптом: "
        + ", ".join(missing))
    for section in ("## Порядок работ", "### Угол архитектуры",
                    "### Угол визуала", "### Угол сценариев"):
        assert section in board, f"из борда пропал раздел «{section}»"
