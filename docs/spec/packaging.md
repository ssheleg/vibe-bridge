...
Версий в `SHELL_PROVIDES` нет намеренно: релизов оболочки с тегом не было ни
одного (`git tag -l 'shell-v*'` пуст), а знать наверняка можно и без версии —
payload видит сам факт получения. Гейты держат обе половины: оболочка
