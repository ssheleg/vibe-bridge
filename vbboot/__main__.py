...
def main() -> int:
    ...
    if chosen.fell_back:
        # Обещание «подробности в журнале» стало правдой только сейчас:
        # писать туда умеет МОСТ, и он это делает, получив `chosen`. Раньше
        # оболочка отсылала владельца к записи, которой не существовало —
        # vbboot по построению без зависимостей и без журнала (F-5).
        _complain(f"обновление {chosen.version} не запустилось — мост "
                  f"работает на предыдущей версии; подробности в журнале")

    # Оболочка ЗНАЕТ, какой код выбрала, — и говорит это мосту, а не
    # выбрасывает. Дальше журнал и панель показывают ответ, а не догадку.
    run(chosen)
    return 0
