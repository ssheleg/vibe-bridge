...
#: превращается работа моста, если не передала.
#:
#: Версий здесь нет намеренно, и причина ИСПРАВЛЕНА 2026-09-02. Сначала было
#: написано «релизов оболочки с тегом не было ни одного» со ссылкой на пустой
#: `git tag -l 'shell-v*'` — это оказалось НЕПРАВДОЙ: на удалённом девятнадцать
#: релизов `shell-v*`, а локальная команда молчала лишь потому, что клон эти
#: теги не забирал. Измерять надо было `git ls-remote --tags`.
#:
#: Настоящая причина обойтись без версий сильнее прежней: payload видит САМ
#: ФАКТ получения. Номер версии сказал бы, что оболочка ДОЛЖНА уметь;
#: полученный аргумент говорит, что она умеет — а между «должна» и «умеет»
#: стоит ровно тот разрыв, ради которого этот модуль и написан.
#:
